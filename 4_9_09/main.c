...
void Balancing_Task(void)
{
    ...
    unsigned long last_timestamp = 0; // dt 계산용
    short sensors;
    unsigned char more;
    float q0, q1, q2, q3; 
    ...
    {   
        ...
        if (mpu_data_ready == 1) 
        {
            ...
            if (fifo_status == 0) {
                ...
                // [보완 1] 실제 경과 시간(dt) 계산
                if (last_timestamp != 0) {
                    dt = (float)(timestamp - last_timestamp) / 1000.0f; // ms -> sec
                }
                last_timestamp = timestamp;

                // dt 예외 처리 (통신 오류 등으로 값이 튈 때 대비)
                ...
                static int cnt = 0;
                if(cnt++ % 10 == 0) printf("Angle: %.2f | Out: %.2f\n", current_angle, PID_output);
            }
            else {
